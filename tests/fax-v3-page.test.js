...
test('Fax v3 desktop/mobile: contacts, batches, receipts, history, legacy disposal and employee changes',{skip:!browserPath,timeout:90000},async t=>{
  ...
  for(const width of [1280,390]){
    ...
    await choose();const before=sendCount;await ev("document.getElementById('lastFour').value='123';document.getElementById('send').click()");await pause(150);assert.equal(sendCount,before);
    ...
    await cdp('Page.reload');await until("document.querySelectorAll('#faxHistory li').length===2");
