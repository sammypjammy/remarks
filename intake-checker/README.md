...
Limitations: plain-text labels without bold formatting, tables, HTML, multiple fields on the same line, and alternate heading conventions are not supported. A standalone bold line within a free-text answer is interpreted as a section; other unmarked continuation text is treated as part of the previous value. Actual DeLorean variants should be checked against the debug view before adding validation rules.

## V1 validation

Flow: `parser.js` → `validation.js` → UI report. The parser is unchanged. `rules.js` centralizes exact required/optional labels, record recognition, and the deferred prior-marriage rule. `validateIntake(intake, rules, { now })` returns structured issues with section, record when applicable, field, severity, reason, and a record location. The default clock is the user's local date; tests inject a fixed date.
