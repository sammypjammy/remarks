...
export async function readOutboundFax(messageId) {
  ...
  if (String(data.id) !== messageId || data.type !== "Fax" || data.direction !== "Outbound") {
    const error = new Error("The returned record could not be verified as this outbound fax.");
    error.status = 502;
    throw error;
  }
...
export default async function handler(req, res) {
  ...
  try {
    const { response, data } = await readOutboundFax(messageId);
